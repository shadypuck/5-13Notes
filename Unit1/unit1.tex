...
\begin{itemize}
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}
            ...
            \draw [|-|] (-5,2.4) -- node[above]{\ce{OH} / \ce{NH} (broad)} (-1,2.4);
        \end{tikzpicture}
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}[text height=1.5ex,text depth=0.25ex,every node/.style=black]
            ...
            \draw [blx,semithick] (-12,0)
                ...
                to[out=0,in=180,out looseness=0.06,in looseness=0.1] (-7.22,0)
                -- (-3.69,0)
                to[out=0,in=180,looseness=0.8] (-3.49,0.3) node[below=0.3cm]{3.49} node(3)[above]{1H}
                to[out=0,in=180,looseness=0.8] (-3.29,0)
                -- (-1.1,0)
                to[out=0,in=180,out looseness=0.05,in looseness=0.03] (-1.05,2.3) node[below=2.3cm]{1.05} node(1)[above]{3H}
                to[out=0,in=180,out looseness=0.03,in looseness=0.05] (-1,0)
                -- (0,0)
            ;
            ...
            \node (M) [minimum height=1.4cm] at (-6,3) {\chemfig[atom sep=1.4em,baseline=-1mm]{H-[:30]O-[:-30]C(<[:-30]H)(<:[:-90]H)-[:30]H}}
                ...
                (M.172) edge[out=-150,in=120,->,shorten <=2pt] (3)
            ;
    ...
    \end{figure}
    \begin{itemize}
        \item In this example, there is free rotation around the \ce{C-O} single bond. This rotation puts all of the methyl protons in the chemical environment, i.e., they are chemically equivalent. Thus, because they are chemically equivalent they all have the identical resonance frequency of 1.05 and all contribute to that peak.
        \begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize}
    \item Note that identical protons do not couple themselves.
    \begin{itemize}
