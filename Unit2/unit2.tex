...
\begin{itemize}
    \item \marginnote{10/16:}Lecture 16 recap: Considerations for electrocyclizations.
    \begin{itemize}
        \item Woodward-Hoffmann rules.
        \begin{itemize}
    ...
    \begin{figure}[h!]
        ...
        \schemestart
            \chemfig{*6([4]\charge{30={\tiny\color{grx}$3$}}{}=[@{1}]\charge{90={\tiny\color{grx}$2$}}{}-[@{2}]@{1C1}\charge{90={\tiny\color{grx}$1$}}{}(-R)-[@{3}]@{1C1'}\charge{-150={\tiny\color{grx}$1'$}}{}-[@{4}]\charge{-90={\tiny\color{grx}$2'$}}{}=[@{5}]\charge{-30={\tiny\color{grx}$3'$}}{}-[@{6},,,,opacity=0])}
            \arrow{<=>[\SI{200}{\celsius}]}[,1.2]
            \chemfig{*6([4]@{2C3}\charge{30={\tiny\color{grx}$3$}}{}-\charge{90={\tiny\color{grx}$2$}}{}=\charge{90={\tiny\color{grx}$1$}}{}(-R)-[,,,,opacity=0]\charge{-150={\tiny\color{grx}$1'$}}{}=\charge{-90={\tiny\color{grx}$2'$}}{}-@{2C3'}\charge{-30={\tiny\color{grx}$3'$}}{}-)}
        \schemestop
        \chemmove{
            \draw [curved arrow={4pt}{2pt}] (1) to[bend right=60,looseness=1.5] (6);
            \draw [curved arrow={2pt}{2pt}] (3) to[bend right=60,looseness=1.5] (2);
            \draw [curved arrow={4pt}{2pt}] (5) to[bend right=60,looseness=1.5] (4);
            \draw [blx,-,line width=1.5mm,line cap=round,opacity=0.3] (1C1) -- (1C1');
            \draw [blx,-,line width=1.5mm,line cap=round,opacity=0.3] (2C3) -- (2C3');
        }
    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[b]{0.17\linewidth}
            \centering
            \chemleft{[}
                \chemfig{*6(@{C1}-@{C2}-@{C3}-[,,,,dashed]@{C4}-@{C5}-@{C6}-[,,,,dashed])}
            \chemright{]^\ddagger}
            \chemmove{
                \draw [-,dashed]
                    ($(C1)+(  30:2.5pt)$) -- ($(C2)+( 90:2.5pt)$)
                    ($(C2)+(  90:2.5pt)$) -- ($(C3)+(150:2.5pt)$)
                    ($(C4)+(-150:2.5pt)$) -- ($(C5)+(-90:2.5pt)$)
                    ($(C5)+( -90:2.5pt)$) -- ($(C6)+(-30:2.5pt)$)
                ;
            }
            \caption{True TS.}
        ...
        \begin{subfigure}[b]{0.17\linewidth}
            \centering
            \chemleft{[}
                \chemfig{*6(\charge{90=\.}{}-=-[,,,,opacity=0]=-\charge{-90=\.}{})}
            \chemright{]^\ddagger}
            \caption{Radical TS.}
            \label{fig:Copeorb3Db}
        \end{subfigure}
        \begin{subfigure}[b]{0.17\linewidth}
            \centering
            \chemfig{?@{C1}-[:90,,,,opacity=0]@{C2}-[:120]-[:-20]@{C3}-[:-90,,,,opacity=0]@{C4}-[:-60]?}
            \chemmove{
                \draw [thick,orx] ($(C1)+(0.0018,0)$) to[bend right=120,looseness=300] ($(C1)+(-0.0018,0)$) -- cycle;
                \filldraw [thick,draw=orx,fill=ory] ($(C1)+(0.0018,0)$) to[bend left=120,looseness=300] ($(C1)+(-0.0018,0)$) -- cycle;
                \draw [thick,orx] ($(C4)+(0.0018,0)$) to[bend left=120,looseness=300] ($(C4)+(-0.0018,0)$) -- cycle;
                \filldraw [thick,draw=orx,fill=ory] ($(C4)+(0.0018,0)$) to[bend right=120,looseness=300] ($(C4)+(-0.0018,0)$) -- cycle;
                \draw [thick,orx] ($(C2)+(0.0018,0)$) to[bend left=120,looseness=300] ($(C2)+(-0.0018,0)$) -- cycle;
                \filldraw [thick,draw=orx,fill=ory] ($(C2)+(0.0018,0)$) to[bend right=120,looseness=300] ($(C2)+(-0.0018,0)$) -- cycle;
                \draw [thick,orx] ($(C3)+(0.0018,0)$) to[bend right=120,looseness=300] ($(C3)+(-0.0018,0)$) -- cycle;
            ...
            \caption{Chair TS.}
            \label{fig:Copeorb3Dc}
        \end{subfigure}
        \begin{subfigure}[b]{0.17\linewidth}
            \centering
            \chemfig{?@{C1}-[:90,1.2,,,opacity=0]@{C2}-[:120]-[:-20]@{C3}-[:-90,1.2,,,opacity=0]@{C4}-[:160]?}
            \chemmove{
                \draw [thick,orx] ($(C1)+(0.0018,0)$) to[bend right=120,looseness=300] ($(C1)+(-0.0018,0)$) -- cycle;
                \filldraw [thick,draw=orx,fill=ory] ($(C1)+(0.0018,0)$) to[bend left=120,looseness=300] ($(C1)+(-0.0018,0)$) -- cycle;
                \draw [thick,orx] ($(C4)+(0.0018,0)$) to[bend left=120,looseness=300] ($(C4)+(-0.0018,0)$) -- cycle;
    ...
    \begin{itemize}
        ...
        % \footnote{Essentially, we can conceptualize this mechanism as the initial $1$-$1'$ $\sigma$-bond homolytically cleaving into two allyl radicals with their radical electrons at the $1$ and $1'$ positions, these allyl radicals switching to their resonance form with the radical electron at the $3$ and $3'$ positions, and these new resonance forms coupling to make the new $3$-$3'$ $\sigma$-bond.}
        \begin{itemize}
            \item Recall the MOs of an allyl radical from Figure \ref{fig:allylMO}.
            \item These MOs tell us that both allyl radicals will interact with their SOMO.
        \end{itemize}
        \item We can then draw out these SOMOs in 3D, interacting through either a \emph{chair} (Figure \ref{fig:Copeorb3Dc}) or a \emph{boat} (Figure \ref{fig:Copeorb3Dd}) transition state.
        \begin{itemize}
            \item In each case, we get good overlap at both the bond-breaking and bond-forming positions.
            \item However, since chair conformations are usually more stable than boat conformations (as you should recall from 5.12), the chair transition state will usually be more stable than the boat transition state.
            \item Note that we mention the boat transition state at all because even though it's less favorable, it can still happen.\footnote{Specifically, it can lead to some alternate stereoisomers as minor side products; see Figure \ref{fig:Copestereochem}.}
        \end{itemize}
        \item These transition states are super important because they're how we predict stereochemistry!
        \item Note that we can also split the system into a cation and an anion, and we'll still get good orbital overlap and all the same transition-state stereochemistry.
        \begin{itemize}
            \item Indeed, splitting into two radicals is more of a convention.
            \item You can (and should) try drawing this cation/anion scheme out!!
            \item The fact that it works both ways is yet more evidence that MOs are a good, meaningful model.
        \end{itemize}
    \end{itemize}
    \item Stereochemistry of the Cope rearrangement.
    \begin{figure}[h!]
        \centering
        \footnotesize
        \schemestart
            \chemfig{*6([4]=-(<:Me)-(<Me)-=)}
            \arrow{0[\small $\equiv$][][-2mm]}
            \chemfig{(-[2,0.8]H)(-[:-170])-[@{2}:-20]=_[@{1,0.3}:10]-[@{6}:-130,,,,opacity=0]=_[@{5}:160]-[@{4}:-170](-[:160])(-[6,0.8]H)-[@{3}:50]}
            \arrow
            \chemfig{(-[2,0.8]H)(-[:-170])=_[:-20]-[:10]-[:-130]-[:160]=_[:-170](-[:160])(-[6,0.8]H)}
            \arrow{0[\small $\equiv$][][-2mm]}
            \chemfig{Me-[:30]*6(=---=(-Me))}
        \schemestop
        \chemmove{
            \draw [curved arrow={4pt}{0pt}] (1) to[bend right=50,looseness=1.5] (6);
            \draw [curved arrow={2pt}{2pt}] (3) to[bend right=10,looseness=1.2] (2);
            \draw [curved arrow={4pt}{2pt}] (5) to[bend right=50,looseness=1.5] (4);
        }
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item If it helps to draw in the hydrogens, you should feel free to.
    \end{itemize}
    \pagebreak
    \item As mentioned in Figure \ref{fig:copeRearr}, the Cope rearrangement is an equilibrium reaction. But for it to be synthetically useful, we need to be able to drive the equilibrium toward starting materials or products. How can we do this?
    \begin{figure}[H]
        \centering
        \footnotesize
        \begin{subfigure}[b]{0.49\linewidth}
            \centering
            \schemestart
                \chemfig{*6([4]=-(*3(--))(<:[2]H)-(<:[6]H)-=)}
                \arrow{->[\SI{20}{\celsius}]}[,1.2]
                \chemfig{*7([:-12.857]-=---=-)}
            \schemestop
            \caption{Release of ring strain.}
            \label{fig:CopeEqa}
        \end{subfigure}
        \begin{subfigure}[b]{0.49\linewidth}
            \centering
            \setchemfig{autoreset cntcycle=false}
            \schemestart
                \chemfig{*6(@{1C1a}=@{1C2a}-@{1C3a}=@{1C4a}(-*6(@{1C1b}-@{1C2b}=@{1C3b}-[,,,,opacity=0]@{1C4b}=@{1C5b}-@{1C6b}-))-@{1C5a}=@{1C6a}-)}
                \arrow{<->>[\SI{170}{\celsius}]}[,1.2]
                \chemfig{*6(@{2C1a}=@{2C2a}-@{2C3a}=@{2C4a}(-*6(@{2C1b}=@{2C2b}-@{2C3b}-@{2C4b}-@{2C5b}=@{2C6b}))-@{2C5a}=@{2C6a}-)}
            \schemestop
            \chemmove{
                \draw [blx,-,line width=3mm,line cap=round,opacity=0.3] (1C2b) -- (1C3b);
                \draw [blx,-,line width=3mm,line cap=round,opacity=0.3] (1C4b) -- (1C5b);
                \draw [blx,-,line width=3mm,line cap=round,line join=round,opacity=0.3]
                    (1C1a.center) -- (1C2a.center) -- (1C3a.center) -- (1C4a.center) -- (1C5a.center) -- (1C6a.center) -- cycle
                    -- ($(1C2a)!0.5!(cyclecenter2)$) -- ($(1C3a)!0.5!(cyclecenter2)$) -- ($(1C4a)!0.5!(cyclecenter2)$) -- ($(1C5a)!0.5!(cyclecenter2)$) -- ($(1C6a)!0.5!(cyclecenter2)$) -- ($(1C1a)!0.5!(cyclecenter2)$)
                    -- (1C4a)
                ;
                % 
                \draw [blx,-,line width=3mm,line cap=round,opacity=0.3] (2C5b) -- (2C6b);
                \draw [blx,-,line width=3mm,line cap=round,opacity=0.3]
                    (2C2b) -- (2C1b) -- (2C4a) -- (2C5a) -- (2C6a) -- (2C1a) -- (2C2a) -- (2C3a) -- (2C4a)
        ...
        \begin{subfigure}[b]{0.45\linewidth}
            \centering
            \schemestart
                \chemfig{*6([4]=-(-\charge{[extra sep=5pt]90=$\oplus$}{K}\charge{45=$\ominus$}{O})--=)}
                \arrow{->[rt]}
                \chemfig{*6(=----(=_O))}
            \schemestop
    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[b]{0.2\linewidth}
            \centering
            \chemfig{*8(-[,,,,blx]-[,,,,blx]-[,,,,blx](-[2,1.2,,,blx]-[::60,1.4,,,grx]-[::60,1.4,,,grx]-[::60,1.2,,,blx]))}
            \caption{7-membered.}
    ...
    % \begin{itemize}
    %     \item You will need these skills to be sharp for predicting the stereochemistry of sigmatropic rearrangements.
    % \end{itemize}
    ...
    \end{itemize}
    \item The Claisen rearrangement can be accelerated by adding different substituents in the \ce{X} position.
    \begin{table}[h!]
        \centering
        \small
        \renewcommand{\arraystretch}{1.2}
    ...
    \begin{figure}[H]
        ...
        \schemestart
            \chemleft{[}\subscheme{
                \chemfig{*6([:120]=--O-*6(=-=-=-))}
                \arrow{<->}[,1.2]
                \chemfig{*6([:60]-[@{6},,,,opacity=0]=[@{1}]-[@{2}]-[@{3,0.7}]O-[@{4,0.3}]*6(-=-=-=[@{5}]))}
            }\chemright{]}
            \arrow{->[\SI{200}{\celsius}]}[,1.2]
            \chemfig{*6(-=-(*6(--=))-(=O)-=)}
            \arrow{->[taut.]}[,1.2]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Then we can push arrows in our Claisen rearrangement to yield a nonaromatic intermediate.
        \end{itemize}
        \item This intermediate then tautomerizes into an enol.
        \begin{itemize}
            \item Enols are usually less stable than ketones, but this enol is aromatic.\footnote{Technically, we call aromatic enols, "phenols."} Therefore, it's favored.
        \end{itemize}
        \item This Claisen rearrangement happens at \SI{200}{\celsius} because it's not quite as thermodynamically downhill as some others.
    \end{itemize}
    \item Stereochemistry of the Claisen rearrangement.
    \begin{figure}[h!]
        \centering
        \footnotesize
        \schemestart
            \chemfig{*6((-[,0.8]R)=-@{1C}(-[:-50,0.8]R')(<[:-10,0.8]H)<:O-(-[,0.8]OEt)=)}
            \arrow{0[\small $\equiv$][][-2mm]}
            \chemfig[fixed length=false]{(-[2,0.9]OEt)-[@{2,0.7}:-20]O-[@{1}:10]@{2C}(-[2,0.9]H)(-[:-20,0.9]R')-[@{6}:-130]=^[@{5}:160](-[2,0.9,,,ovbnd]H)(-[:-130,0.9]R)-[@{4}:-170,,,,opacity=0]=^[@{3}:50]}
            \arrow
            \chemfig[fixed length=false]{(-[2,0.9]OEt)=_[:-20]O-[:10,,,,opacity=0](-[2,0.9]H)(-[:-20,0.9]R')=^[:-130]-[:160]@{3C}(-[2,0.9,,,ovbnd]H)(-[:-130,0.9]R)-[:-170]-[:50]}
            \arrow{0[\small $\equiv$][][-2mm]}
            \chemfig{EtO-[:30](=[2]O)-[:-30]-[:30]@{4C}(<[2]R)-[:-30]=_[:30]-[:-30]R'}
        \schemestop
        \chemmove{
            \draw [curved arrow={2pt}{2pt}] (1) to[bend right=40,looseness=1.3] (2);
            \draw [curved arrow={4pt}{2pt}] (3) to[out=160,in=-130,looseness=8] (4);
            \draw [curved arrow={4pt}{2pt}] (5) to[out=-80,in=-40,looseness=3] (6);
            % 
            \fill [blx,opacity=0.3] (1C) circle (1.5mm);
            \fill [blx,opacity=0.3] (2C) circle (1.5mm);
            \fill [rex,opacity=0.3] (3C) circle (1.5mm);
            \fill [rex,opacity=0.3] (4C) circle (1.5mm);
        }
        \caption{Claisen rearrangement stereochemistry.}
        \label{fig:Claisenstereochem}
    \end{figure}
    \begin{itemize}
        ...
        \begin{itemize}
            \item This time, the way to make the most things equatorial is to put the two \ce{R} groups equatorial and leave the ethoxy group axial.
    ...
    \begin{itemize}
        \item Note that chemists refer to these reactions interchangeably as "hydrogen atom shifts," "\ce{H}-atom shifts," "\ce{H} shifts," etc.
        \item We can also have sigmatropic methyl shifts, though we won't discuss these explicitly this lecture.
    \end{itemize}
    \item Example: A $[1,3]$ \ce{H}-atom shift.
    \begin{figure}[h!]
        \centering
        \footnotesize
        \begin{subfigure}[b]{\linewidth}
            \centering
            \schemestart
                \chemfig{Me-[:30](<[@{1}2]H)-[@{2}:-30]=^[@{3}:30]@{C}-[:-30]Et}
                \arrow{->[$\Delta$]}
                \chemfig{Me-[:30]=^[:-30]-[:30](-[2,,,,wvbnd]H)-[:-30]Et}
            \schemestop
            \chemmove{
                \draw [curved arrow={3pt}{2pt}] (1) to[out=20,in=90] (C);
                \draw [curved arrow={4pt}{2pt}] (3) to[bend right=60,looseness=1.8] (2);
            }
            \caption{A $[1,3]$ \ce{H}-shift.}
            \label{fig:Hshift13a}
        \end{subfigure}\\[2.5em]
        \begin{subfigure}[b]{\linewidth}
            \centering
            \schemestart
                \chemfig{Me-[:30]\charge{90=\.}{}(-[2,,,,opacity=0]\charge{-90=\.}{H})-[:-30]=^[:30](-[6,,,,opacity=0]\phantom{H})-[:-30]Et}
                \arrow{0[\small $\equiv$][][-2mm]}
                \chemfig{Me-[:30]@{2C1}(-[2,,,,opacity=0]@{2H}H)-[:-30]-[:30]@{2C3}(-[6,,,,opacity=0]\phantom{H})-[:-30]Et}
                \arrow
                \chemfig{Me-[:30]@{3C1}-[:-30]-[:30]@{3C3}(-[6,,,,opacity=0]@{3H}H)(-[2,,,,opacity=0]\phantom{H})-[:-30]Et}
                \arrow{0[\small $\equiv$][][-2mm]}
                \chemfig{Me-[:30]=^[:-30]-[:30](<:[2]H)(-[6,,,,opacity=0]\phantom{H})-[:-30]Et}
            \schemestop
            \chemmove{
                \draw [thick,orx] ($(2C1)+(0.0018,0)$) to[bend left=120,looseness=540] ($(2C1)+(-0.0018,0)$) -- cycle;
                ...
                \draw [thick,orx] ($(2C3)+(0.0018,0)$) to[bend right=120,looseness=540] ($(2C3)+(-0.0018,0)$) -- cycle;
                \filldraw [thick,draw=orx,fill=ory] ($(2C3)+(0.0018,0)$) to[bend left=120,looseness=540] ($(2C3)+(-0.0018,0)$) -- cycle;
                \draw [cyan,thick,-] ([xshift=-1.5mm,yshift=3mm]2C1.center) to[bend left=50] node[left]{$\checkmark$} ([xshift=-2mm,yshift=-1.5mm]2H.center);
                \draw [cyan,thick] (2Hn) to[out=-30,in=180] node[above right=-1mm]{$\checkmark$} ([xshift=-1.5mm,yshift=-3mm]2C3.center);
                \draw [rex,thick] (2Hn) to[out=30,in=120] node[above]{X} ([xshift=-1.5mm,yshift=5mm]2C3.center);
                % 
                \draw [thick,orx] ($(3C1)+(0.0018,0)$) to[bend left=120,looseness=540] ($(3C1)+(-0.0018,0)$) -- cycle;
                ...
                \draw [thick,orx] ($(3C3)+(0.0018,0)$) to[bend right=120,looseness=540] ($(3C3)+(-0.0018,0)$) -- cycle;
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item In particular, the \ce{H} can't just jump to the other top orbital because said top orbital has the wrong shading.
            \end{itemize}
        \end{itemize}
        \item Takeaway: For \ce{H} to move, it has to cross to the other face of the molecule.
        \begin{itemize}
    ...
    \item Example: Consider a $[1,5]$ \ce{H}-atom shift.
    \begin{figure}[H]
        \centering
        \footnotesize
        \begin{subfigure}[b]{0.35\linewidth}
            \centering
            \schemestart
                \chemfig{*6(=(--[,0.6,,,opacity=0])-=--H)}
                \arrow{->[$\Delta$]}
                \chemfig{*6([:-60]H--(--[,0.6,,,opacity=0])=-=)}
            \schemestop
            \caption{A $[1,5]$ \ce{H}-shift.}
            \label{fig:Hshift15a}
        \end{subfigure}
        \begin{subfigure}[b]{0.6\linewidth}
            \centering
            \schemestart
                \chemfig{*6(=(-)-=-\charge{-150=\.}{}-[,,,,opacity=0]\charge{[extra sep=2pt]30=\.}{H})}
                \arrow{0[\small $\equiv$][][-2mm]}
                \chemfig{*5([:-45]@{2C1}-(-)-@{2C3}--@{2C5}(-[2]@{2H}H))}
                \arrow
                \chemfig{*5([:-45]@{3C1}(-[2]@{3H}H)-(-)-@{3C3}--@{3C5})}
                \arrow{0[\small $\equiv$][][-2mm]}
                \chemfig{*6([:-60]H--(-)=-=)}
            \schemestop
            \chemmove{
                \draw [thick,orx] ($(2C5)+(0.0018,0)$) to[bend left=120,looseness=540] ($(2C5)+(-0.0018,0)$) -- cycle;
                ...
                \filldraw [thick,draw=orx,fill=ory] ($(2C3)+(0.0018,0)$) to[bend left=120,looseness=540] ($(2C3)+(-0.0018,0)$) -- cycle;
                \draw [cyan,thick] (2Hn) to[out=150,in=90] node[above left=-1mm]{$\checkmark$} ([yshift=5mm]2C1.center);
                % 
                \draw [thick,orx] ($(3C1)+(0.0018,0)$) to[bend left=120,looseness=540] ($(3C1)+(-0.0018,0)$) -- cycle;
                \filldraw [thick,draw=orx,fill=ory] ($(3C1)+(0.0018,0)$) to[bend right=120,looseness=540] ($(3C1)+(-0.0018,0)$) -- cycle;
                \node [circle,thick,draw=orx,fill=ory,inner sep=1pt] at (3H) {H};
                ...
                \filldraw [thick,draw=orx,fill=ory] ($(3C3)+(0.0018,0)$) to[bend left=120,looseness=540] ($(3C3)+(-0.0018,0)$) -- cycle;
                \draw [thick,orx] ($(3C5)+(0.0018,0)$) to[bend left=120,looseness=540] ($(3C5)+(-0.0018,0)$) -- cycle;
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item We predict the product stereochemistry by continuing to use the same method as before.
        \begin{itemize}
            \item This time, we split into a pentadienyl radical (5 $p$-orbitals and 5 electrons) to complement our hydrogen radical (1 $s$-orbital and 1 electron).
            \item The HOMO of the pentadienyl radical will be the 3rd energy level, so it will have 2 symmetric nodes.
            \begin{itemize}
                \item It may be helpful to draw the HOMO on a straight line first (as in Figure \ref{fig:dieneMO}) and then "wrap it around" onto the intermediate drawn in Figure \ref{fig:Hshift15b}.
            \end{itemize}
            \item With the orbitals drawn, we must (once again) move the hydrogen to another lobe with the right shading.
            \begin{itemize}
                \item This time, however, the \ce{H} can just jump directly over from shaded to shaded.
                \item This is suprafacial movement!
            \end{itemize}
        \end{itemize}
        \item Practically speaking, suprafacial moves are common.
        \begin{itemize}
            \item $[1,5]$ H-atom shifts happen frequently, whether we like it or not!
            \item This is a real thing: When we try to make a product in the lab, we often access conditions in which the hydrogen will just dance back and forth.
            \item Indeed, there are isotopic labeling studies in which a molecule with deuterium (\ce{D}) on one position will engage in $[1,5]$ \ce{H}/\ce{D}-shifts to such an extent that the deuterium will become scrambled (i.e., equally distributed between the two possible positions) over time.
            \item See \textcite{bib:Clayden} for more.
        \end{itemize}
    \end{itemize}
    \item We can do all of these rearrangements photochemically as well, instead of thermally.
    \begin{itemize}
        \item This will lead to the opposite things from before, because we now have a new HOMO.
        \item Implication: Photochemical $[1,3]$ shifts will be suprafacial and photochemical $[1,5]$ shifts will be antarafacial.
        \item You should try drawing these new orbitals and arrangements out!!
    \end{itemize}
    \item Note: \textcite{bib:Clayden} says that $[1,3]$ suprafacial is thermally forbidden, but practically, there is a small possibility that this will happen.
\end{itemize}
...
\begin{itemize}
    \item \marginnote{10/18:}Lecture 14 recap: Sigmatropics (our final pericyclic reaction).
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item $[1,3]$ \ce{H}-shifts: Antarafacial when thermal (Figure \ref{fig:Hshift13b}), and suprafacial when photochemical.
            \begin{itemize}
                \item Photochemical $[1,3]$ \ce{H}-shifts are suprafacial because the allyl radical reacts with top MO in Figure \ref{fig:allylMO}, which has like-shaded orbitals at both ends!
            \end{itemize}
        \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Example: The \ce{C-I} single bond.
        \begin{itemize}
            \item There is a $\sigma$-orbital and a $\sigma^*$-orbital.
            \item The $\sigma$-orbital is polarized toward \ce{I}, and the $\sigma^*$-orbital is polarized toward \ce{C}.
        \end{itemize}
        \item Example: The \ce{C=O} double bond.
        \begin{itemize}
            ...
            \item The $\pi$-orbital is polarized toward \ce{O}, and the $\pi^*$-orbital is polarized toward \ce{C}.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item MO explanation: The lone pair on the heteroatom in the ring donates into the $\sigma^*$-orbital of the axial substituent.
            \begin{itemize}
                \item This is $n_{\ce{O}}\to\sigma_{\ce{CO}}^*$ donation; review your hyperconjugation notation!!
            \end{itemize}
        \end{itemize}
        \pagebreak
        \item Example of how hyperconjugation manifests: Effects on structure.
        \begin{itemize}
            \item Example: The staggered vs. eclipsed conformations of ethane (Figure \ref{fig:ethaneStaggered}).
        \end{itemize}
        \item Example of how hyperconjugation manifests: Stability of carbocations.
        \begin{itemize}
            \item Example: Tertiary vs. primary (Figure \ref{fig:hyperconjugationCC}).
        \end{itemize}
        \item Example of how hyperconjugation manifests: Reactivity.
        \begin{itemize}
            \item Example: The rates of S\textsubscript{N}1 reactions (Figure \ref{fig:stereoelectronicRxn}).
        \end{itemize}
        \item Takeaway: Key notions for hyperconjugation.
        \begin{itemize}
            \item Know when it's there.
            \item Be able to draw the donor and acceptor orbitals.
            \item Understand the geometric factors, e.g., antiperiplanar donor-acceptor interactions.
        \end{itemize}
    \end{itemize}
    \item Key takeaways: Pericyclics.
    \begin{itemize}
        \item Know your nomenclature; Prof. Elkin won't repeat it here, though.
        \item Big takeaway from this lecture: The rules for drawing the MOs of conjugated systems.
        \begin{itemize}
            \item We have used this technique in basically every lecture since it was introduced.
            \item This is arguably the \emph{key} concept in this unit.
        \end{itemize}
        \item Example: Butadiene (Figure \ref{fig:dieneMO}).
        \begin{itemize}
            \item The rules.
            \begin{itemize}
                \item The number of nodes always starts with zero and goes up by 1 at every energy level.
                \item Nodes are drawn symmetrically.
                \item At the highest level, you always have one node between all adjacent orbitals.
                \item The teaching team often gets questions about how to draw these; make sure to practice!!
            \end{itemize}
            \item Know how to populate orbitals as well.
            \begin{itemize}
                \item We've got 4 $\pi$-electrons, which we populate starting from the bottom per the Aufbau principle, the Pauli exclusion principle, and Hund's rule.
            \end{itemize}
        \end{itemize}
        \item Light excites one electron up one energy level (Figure \ref{fig:thermPhot}).
        \item The key to understanding most pericyclic reactions is to draw the HOMO, the LUMO, or both for conjugated system(s).
    \end{itemize}
    \item Prof. Elkin pauses to ask for questions on MOs.
    \item For the anomeric effect, do we need to know anything besides the axial preference? For example, do we need to know about its effect on the rate of reaction, etc.?
    \begin{itemize}
        \item By definition, the "anomeric effect" is the favoring of the axial substituent.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Multiple product options from \ce{Me2S}, \ce{H2O2}, or \ce{NaBH4} as second-step additives (Figure \ref{fig:ozonideAcid}).
        \end{itemize}
